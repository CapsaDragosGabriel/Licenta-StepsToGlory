...
public class RangedAIEvolved : MonoBehaviour, IEffecteable
{
    ...
    [SerializeField]
    private GameObject enemyRoAPrefab;

    private bool canUseRoA = true;
    private bool isUsingRoA = false;
    private float roaCooldown = 3f;
    //private float currRoATime = 0f;
    private float roaDamage = 2f;
    private float roaActiveTime = 3f;
    private float roaTickRate = 1.5f;
    ...
    void CastRoA()
    {
        if (target != null)
        {
            GameObject roaInstance = Instantiate(enemyRoAPrefab);
            roaInstance.GetComponent<Transform>().position= target.transform.position; 
            roaInstance.GetComponent<enemyRoA>().SetDamage(roaDamage);
            roaInstance.GetComponent<enemyRoA>().SetTickRate(roaTickRate);
            roaInstance.GetComponent<enemyRoA>().StartDamage();

            isUsingRoA = true;
            canUseRoA= false;
            StartCoroutine("ResetRoa");
            
            Destroy(roaInstance,roaActiveTime);
        }
    }

    public IEnumerator ResetRoa()
    {
        yield return new WaitForSeconds(roaActiveTime);
        isUsingRoA= false;
      
        yield return new WaitForSeconds(roaCooldown);
        canUseRoA = true;
    }
    void Shoot()
    {
